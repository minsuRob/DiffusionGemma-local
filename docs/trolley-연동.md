...
**이 저장소에 툴 콜링이 없다.** `server.py`는 백엔드에게 텍스트 생성과 이미지 읽기만
시키고, 에이전트 루프도 함수 호출도 MCP 클라이언트도 없다. 모델 출력이 동작으로
디스패치되는 경로가 존재하지 않는다.
...
이제 모델이 로컬 DiffusionGemma 하나가 아니다. `backends.py`가 백엔드를 갈아끼울 수
있게 만들었고, 그중에는 **툴 콜링을 네이티브로 지원하는 것(Gemini)** 이 있다. 아래 A안의
가장 큰 난관 — "DiffusionGemma가 툴 콜을 안정적으로 뱉게 프롬프트를 설계하는 일" — 은
툴 콜링이 되는 백엔드를 고르는 순간 사라진다.

에이전트 루프를 붙인다면 백엔드 인터페이스에 `stream_tools(messages, tools, ...)`를
하나 더하고, 그것을 구현한 백엔드에서만 루프를 여는 형태가 자연스럽다. 능력 플래그
(`capabilities`)가 이미 그런 식으로 쓰이고 있다 — vision이 없는 백엔드는 첨부 버튼이
잠기는 것과 같은 방식으로.
...
주의할 점은 **우리 쪽 단일 워커 제약**이다. 단 이건 이제 로컬 백엔드에만 해당한다.
`LocalEngine`이 모델 하나·워커 스레드 하나로 로컬 요청을 직렬화하므로(중복 실행 시
~34GB로 OOM), 로컬 백엔드로 에이전트 루프를 돌리면 그동안 다른 로컬 채팅은 큐에서
대기한다(상한 8, 초과 시 503). 루프가 툴을 여러 번 호출하면 그만큼 점유 시간이 길어진다.
원격 백엔드로 루프를 돌리면 `RemoteRunner` 쪽이라 이 제약을 받지 않는다. SSE로 진행 상황을 흘려보내는 기존 패턴을 그대로 쓰면 사용자에게
